...
def main():
    ...
    def get_peers_cmd(os, protocols):
        ...
        if 'Cisco Adaptive Security Appliance Software' in os:
            ...
            print 'not supported yet'
        elif 'Cisco IOS XE Software' in os:
    ...
    def show_peers(remote_conn, os):
        '''
        Get peers for each active protocol.
        '''

        if os == 'Cisco IOS XE Software':
            show_ip_proto_cmd = "show ip protocols summary\n"
        elif os == 'Cisco IOS Software':
            show_ip_proto_cmd = "show ip protocols summary\n"
        elif os == 'Cisco Nexus Operating System (NX-OS) Software':
            show_ip_proto_cmd - "show ip route summary"
        elif os == 'Cisco Adaptive Security Appliance Software':
            show_ip_proto_cmd = "show ip protocols summary\n"
    ...
    for ip in ips:
        ...
        for os in devices_this_script_works_on:
            if os in version:
                if 'Cisco IOS XE Software' in os:
